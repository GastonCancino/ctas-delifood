...
SELECT id_pedido, id_carta_x_fecha, te.id_tipo_ent, te.nombre_tipo_ent, ee.nombre_ent, tc.id_tipo_comida, tc.nombre_tipo_comida,
	pe.id_alm_prog, ap.nombre_alm_prog, cantidad_pedido,  
	tipo_precio_pedido, fecha_reg_pedido,  estado_pedido
	FROM pedido pe 
	INNER JOIN almuerzo_prog ap ON pe.id_alm_prog = ap.id_alm_prog 
	INNER JOIN tipo_comida tc ON ap.id_tipo_comida = tc.id_tipo_comida 
	INNER JOIN entidad ee ON pe.id_ent = ee.id_ent
	INNER JOIN tipo_entidad te ON ee.id_tipo_ent = te.id_tipo_ent
